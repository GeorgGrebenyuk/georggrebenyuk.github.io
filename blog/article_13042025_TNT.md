...
Тот факт, что я пишу эту статью только сейчас, по истечению почти 4 месяцев после начала работ говорит о том, что процесс реализации был мягко говоря не прост, местами я несколько раз бросал из-за сложностей:
...
Выбор C++ в качестве языка разработки был обусловлен тем, что рабочим является только API на этом языке программирования, соответственно для создания удобной среды (включая UI) работы потребовалось в несколько раз больше времени и усилий, чем обычно затрачивается при работе с .NET API. Добавляю сюда же и то, что раннее с Qt на серьезных проектах я не работал и первое время безбожно тупил.
...
Под этой категорией постараюсь рассказать про реализованные команды для изменения стиля отображения облаков точек и анализа общих данных облаков точек.

Возможность в отдельной палитре отобразить в табличном виде пользовательскую схему раскраски и ограничивать видимость точек облака точек по ним.

![](assets/2025-04-13-13-25-44-image.png)

Совместная реализация с плагином TBS Plus по расстановке значения отклонения в виде мультивыносок, ориентирующихся по заданной точке обзора в модели nanoCAD:
...
Авторская (личная) реализация классификации облака точек на выявление наклонных и вертикальных конструкций путем анализа уклонов между соседними точками облака, по этому пишется научная статейка, выйдет ближе к осени:

![](assets/2025-04-13-13-37-51-image.png)

# Геометрическая аналитика

Тут используются алгоритмы геометрической библиотеки с открытым исходным кодом GEOS (используется, в частности, в составе GDAL и многих ГИС-инструментов).
